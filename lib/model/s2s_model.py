...
class S2SModel(nn.Module):
    ...
    def forward(self, input_tensor, target_tensor, criterion, max_length=MAX_LENGTH, teacher_forcing=False):
        '''------------------------------------------------------------
        ...
        ------------------------------------------------------------'''
        encoder_outputs = torch.zeros(max_length, batch_size, self.encoder.hidden_dim)
        ...
        all_decoder_outputs = Variable(torch.zeros(max_length, batch_size, 1))
        all_decoder_outputs = all_decoder_outputs.to(device)

        all_attentions = Variable(torch.zeros(max_length, batch_size, max_length))
        all_attentions = all_attentions.to(device)
        '''------------------------------------------------------------
        9: Start decoding:
